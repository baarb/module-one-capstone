...
public class VendingMachineCLI {
	...
	private String[] SECOND_MENU_OPTIONS = { SECOND_MENU_OPTION_FEED_MONEY, SECOND_MENU_OPTION_SELECT_PRODUCT,
			SECOND_MENU_OPTION_FINISH_TRANSACTION, };

	private static final String[] THIRD_MENU_OPTION_LOCATION = { "A1", "A2", "A3", "A4", "B1", "B2", "B3", "B4", "C1",
	...
	public void run() throws FileNotFoundException {
		...
		DeliveryBin vendingBin = new DeliveryBin();
		String choice = "";

		while (true) {
			if (choice == "") {
				choice = (String) menu.getChoiceFromOptions(MAIN_MENU_OPTIONS,
						"Your current balance is: $" + vendOMatic.getBalance());
			}
			...
			if (choice.equals(MAIN_MENU_OPTION_DISPLAY_ITEMS)) {
				...
				if (secondChoice.equals(SECOND_MENU_OPTION_FEED_MONEY)) {
					...
					if (boughtItem != null) {
						vendingBin.addToDeliveryBin(boughtItem);
					}
					...
					for (Item itemInBin : vendingBin.getDeliveryBin()) {
